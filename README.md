...
- **0.0.6**:修复当日高峰/非高峰拆分。改为对当日查询自带的小时序列(x_time/tokensUsage)求和工作日 14:00–17:59 小时桶,不再单独发峰窗区间请求——官方监控接口对当天的区间查询会把 endTime 截到当前时刻(晚间查询高峰=全天、非高峰=0),且区间 endTime 桶为包含语义(多算 18–19 点)。顺带少一次请求。
